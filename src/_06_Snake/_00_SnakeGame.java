...
public class _00_SnakeGame implements ActionListener, KeyListener {
	...
	public void startGame() {
		...
		String instructions = "use arrow keys to move the snake, eat apples! get as big as possible so you can eat more apples\nchoose difficulty";
	...
	private void setFoodLocation() {
		...
		Location l = new Location(new Random().nextInt(WINDOW_WIDTH)*WINDOW_SCALE, new Random().nextInt(WINDOW_HEIGHT) * WINDOW_SCALE);
		//2. set the foodLocation variable equal to the Location object you just created.
		while (snake.isLocationOnSnake(l)) {
			l = new Location(new Random().nextInt(WINDOW_WIDTH)*WINDOW_SCALE, new Random().nextInt(WINDOW_HEIGHT) * WINDOW_SCALE);
		}
